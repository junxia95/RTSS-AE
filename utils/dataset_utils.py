...
def _get_dataset_targets(dataset):
    ...
    for idx in range(len(dataset)):
        label = dataset[idx][1]
        targets.append(int(label.item()) if hasattr(label, 'item') else int(label))
...
def separate_data(train_data, num_clients, num_classes, beta=0.4):
    ...
    y_train = _get_dataset_targets(train_data)
    #y_train = np.array([image[1] for image in train_data.images])
    min_size_train = 0
    min_require_size = 10
    K = num_classes
