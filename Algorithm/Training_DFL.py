...
import numpy as np
from utils.device_rl import DeviceAggregationRLController
...
class Client(object):
    ...
    def calculate_label_distribuion(self, dataset, args):
        self.label_distribution = np.zeros(args.num_classes)
        ldr_train = DataLoader(DatasetSplit(dataset, self.data_idx), batch_size=args.local_bs, shuffle=True)
        for batch_idx, (images, labels) in enumerate(ldr_train):
            for label in labels:
                self.label_distribution[int(label)] = self.label_distribution[int(label)] + 1
...
def DFL(args, net_glob, dataset_train, dataset_test, dict_users):
    ...
    for idx in range(args.num_users):
        ...
        client.calculate_label_distribuion(dataset_train, args)
    ...
    rl_aggregation_enabled = args.client_selection in ['rl', 'rl_aggregation', 'rl_agg']
    aggregation_rl_controller = None
    if rl_aggregation_enabled:
        aggregation_rl_controller = DeviceAggregationRLController(args, Global_Client_set, Adjacency_matrix, NetWork_type)
    rl_aggregation_rewards = []
    current_time = 0
    current_comm = 0
    current_energy = 0
    for iter in range(args.epochs):
        ...
        for idx in active_clients:
            ...
            comm_time_total = 0
            comm_energy_total = 0
            if aggregation_rl_controller is not None:
                action = aggregation_rl_controller.select_action(idx, battery_state, iter)
                selected_neighbors = action['selected_neighbors']
                selected_quant_bits = action['quant_bits']
            else:
                action = None
                selected_neighbors = [
                ...
                selected_quant_bits = {
                    nb_idx: Global_Client_set[nb_idx].quant_bits for nb_idx in selected_neighbors
                }
            selected_neighbors = [nb_idx for nb_idx in selected_neighbors if nb_idx in active_client_set]
            selected_quant_bits = {
                nb_idx: selected_quant_bits[nb_idx]
                for nb_idx in selected_neighbors
                if nb_idx in selected_quant_bits
            }
            for nb_idx in selected_neighbors:
                neighbor_state, neighbor_payload_bytes, _ = transmit_state_dict(
                ...
                w_locals.append(neighbor_state)
                lens.append(Global_Client_set[nb_idx].data_cnt)
                sender_bits = selected_quant_bits[nb_idx]
                comm_ratio = max(neighbor_payload_bytes / float(base_payload_bytes), 1e-12)
                ...
                comm_time_total = comm_time_total + comm_time
                comm_energy_total = comm_energy_total + src_energy + dst_energy
                consume_communication_energy(battery_state, nb_idx, idx, src_energy, dst_energy)
                round_comm = round_comm + neighbor_payload_bytes / (1024 * 1024)
                round_comm_time = round_comm_time + comm_time
            if aggregation_rl_controller is not None:
                reward = aggregation_rl_controller.observed_reward(
                    idx, action, battery_state, comm_time_total, comm_energy_total
                )
                aggregation_rl_controller.observe(idx, action, reward, battery_state, iter + 1)
                rl_aggregation_rewards.append({
                    'round': iter,
                    'receiver': idx,
                    'selected_neighbors': list(action['selected_neighbors']),
                    'quant_bits': dict(action['quant_bits']),
                    'reward': reward,
                })
            round_time = max(round_time, t)
            own_state, _, _ = transmit_state_dict(
    ...
    if aggregation_rl_controller is not None:
        save_result(aggregation_rl_controller.action_history, 'rl_aggregation_action', args)
        save_result(rl_aggregation_rewards, 'rl_aggregation_reward', args)
        save_result(aggregation_rl_controller.q_history, 'rl_aggregation_q_history', args)
